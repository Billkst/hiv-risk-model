...
class EnhancedPredictor:
    ...
    def _compute_feature_attention(self, X):
        ...
        # 归一化：使总和为1（概率分布）
        feature_attention = feature_attention / feature_attention.sum()
    ...
    def predict_single(self, features_dict, return_attention=False, include_contributions=False):
        ...
        # 获取概率分布（GradientBoosting和RandomForest都支持）
        proba = self.base_model.predict_proba(X_weighted)[0]
        
        # 计算连续风险评分（0-100分）
        # 使用期望值：E[level] = Σ(level_i × P(level_i))
        expected_level = sum((i + 1) * proba[i] for i in range(len(proba)))
        
        # 映射到0-100分：(expected_level - 1) / 4 * 100
        # 等级1(1.0) → 0分, 等级5(5.0) → 100分
        risk_score = (expected_level - 1) / 4 * 100
        
        # 置信度：预测等级的概率
        confidence = float(proba[risk_level - 1])
        
        # 构建结果
        result = {
            'risk_level_5': risk_level,
